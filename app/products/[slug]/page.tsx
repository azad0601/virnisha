...
function buildItemListJsonLd(slug: string, category: { name: string; products: { name: string; cas: string }[] }) {
  return {
...
export default function CategoryPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <section aria-labelledby="category-heading" className="bg-navy py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="max-w-3xl">
            ...
            <h1 id="category-heading" className="text-4xl md:text-5xl font-bold text-white mb-6">
              {category.name}
            </h1>
            <p className="text-blue-200 text-lg leading-relaxed">{category.description}</p>
          </div>
        </div>
      </section>

      {/* Product Table */}
      <section aria-labelledby="products-table-heading" className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Disclaimer */}
          <div className="mb-8 p-4 bg-amber-50 border border-amber-200 rounded-lg">
            <p className="text-amber-800 text-sm leading-relaxed">
              <strong>Note:</strong> This catalogue is for sourcing reference only. Product availability,
              grade, specification, regulatory status, documentation and pricing must be confirmed for each
              individual enquiry.
            </p>
          </div>

          <div className="flex items-center justify-between mb-6">
            <h2 id="products-table-heading" className="text-xl font-bold text-navy">
      ...
      {/* CTA */}
      <section aria-label="Product enquiry call to action" className="py-16 bg-gray-50 border-t border-gray-200">
        <div className="max-w-3xl mx-auto px-4 text-center">
          <h2 className="text-2xl font-bold text-navy mb-3">
            Need a specific grade or quantity?
          </h2>
          <p className="text-gray-600 mb-6">
            Contact us with your product name, CAS number, required quantity and grade for a tailored quotation.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="mailto:virnishaoverseas2026@gmail.com" className="btn-primary">
              Email Enquiry
            </a>
            <a href="tel:+917977119881" className="btn-secondary">
              Call Us
            </a>
          </div>
        </div>
